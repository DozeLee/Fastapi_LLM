...
class PDFProcessor:
    ...
    def get_pdf_chunks_for_rag(self, pdf_path, chunk_size=400, overlap=50):
        """一键生成RAG入库用的文本块（封装全流程）"""
        # 1. 检查文件
        if not self.check_pdf_file(pdf_path):
            return None, None, None

        # 2. 提取内容
        full_text, page_texts = self.extract_pdf_content(pdf_path)
        if not page_texts:
            return None, None, None

        # 3. 分块+过滤+组装元数据（把RAG里的逻辑移到这里）
        documents = []
        metadatas = []
        ids = []
        ...
        for page_info in page_texts:
            ...
            # 分块
            chunks = self.enhanced_chunking(text, chunk_size, overlap)
            # 过滤短块（把RAG里的<20字符过滤移到这里）
        ...
        print(f"✅ RAG入库准备完成: {len(documents)} 个有效文本块")
